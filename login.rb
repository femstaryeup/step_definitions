Given(/^That I am on the electrolux login page$/) do
  @driver.navigate.to ('http://dep3.t1electroluxqa.electrolux.se/en/')
  sleep 7
  @driver.find_element(:id, 'uxModalLogin').click
end


When(/^I enter my user name$/) do
#@browser.text_field(:id => 'Email_14').set 'dep-tester1@headlondon.com'
@driver.find_element(:id, 'Email_14').send_keys 'dep-tester1@headlondon.com'
end


And(/^my password$/) do
#@browser.text_field(:id => 'Password').set 'quality1234'
@driver.find_element(:id, 'Password').send_keys 'quality1234'
end


And(/^I click the login button$/) do
@driver.find_element(:xpath, ".//*[@id='GlobalMenuLogin']/button").click
end


Then(/^I should be logged in successfully$/) do

@driver.find_element(:xpath, "html/body/div[1]/div[1]/div/ul/li[4]/a").click
@driver.find_element(:xpath, "html/body/div[1]/div[1]/div/ul/li[4]/div/ul/li[1]/a").click

#Click my account then verify page title as a logged in user

  expect(@driver.title).to be == 'My Pages | Electrolux - NOT FOR COMMERCIAL USE'
  @driver.find_element(:xpath, "html/body/div[1]/div[1]/div/ul/li[4]/a").click
  @driver.find_element(:xpath, "html/body/div[1]/div[1]/div/ul/li[4]/div/ul/li[3]/a").click

end


When(/^I enter a wrong username$/) do
  @driver.find_element(:id, 'Email_14').send_keys 'dep-teste1@headlondon.com'

end

And(/^a password$/) do
  @driver.find_element(:id, 'Password').send_keys 'quality12345'
end



Then(/^I should not be able to login successfully$/) do
  expect(@driver.title).to be == "Log in to my Owner's Lounge | Electrolux - NOT FOR COMMERCIAL USE"
  expect(@driver.find_element(:tag_name, "body").text).to include("That email address or password isn't right")
  #puts @browser.text.include? "That email address or password isn't right"

end